...
matrixT::operator *= (const T& c) _NO_THROW
{
	...
	for (size_t j=0; j < _m->Col; j++)
	    _m->Val[i][j] *= c;
 	 if (-1e-10<_m->Val[i][j]&& _m->Val[i][j]<0) _m->Val[i][j] = 0;
	 if (1e-10>_m->Val[i][j]&& _m->Val[i][j]>0) _m->Val[i][j] = 0;
   return *this;
}
...
matrixT::operator *= (const matrixT& m) _THROW_MATRIX_ERROR
{
   ...
   for (size_t i=0; i < _m->Row; i++)
      for (size_t j=0; j < m._m->Col; j++)
      {
		 ...
		 if (-1e-10<temp._m->Val[i][j]&& temp._m->Val[i][j]<0) temp._m->Val[i][j] = 0;
		 if (1e-10>temp._m->Val[i][j]&& temp._m->Val[i][j]>0) temp._m->Val[i][j] = 0;
      }
   *this = temp;
...
// summation function
MAT_TEMPLATE inline T
matrixT::Sum () _THROW_MATRIX_ERROR
{
   size_t i,j,k;
   double sum=0;T a1,a2,*rowptr;
